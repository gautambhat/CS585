...
bool searchForMovement(Mat thresholdImage, Mat &cameraFeed){
	...
	for (auto& contour : contours)
	{
		if (contourArea(contour) > 20)
		{
			...
			circle(cameraFeed, centroid, 10, Scalar(0, 255, 0), 2);
			line(cameraFeed, centroid, Point2f(moment.m10 / moment.m00, (moment.m01 / moment.m00) - 15), Scalar(0, 255, 0), 2);
			line(cameraFeed, centroid, Point2f(moment.m10 / moment.m00, (moment.m01 / moment.m00) + 15), Scalar(0, 255, 0), 2);
			line(cameraFeed, centroid, Point2f((moment.m10 / moment.m00) - 15, moment.m01 / moment.m00), Scalar(0, 255, 0), 2);
			line(cameraFeed, centroid, Point2f((moment.m10 / moment.m00) + 15, moment.m01 / moment.m00), Scalar(0, 255, 0), 2);
			putText(cameraFeed, "Tracking object at (" + intToString(centroid.x) + "," + intToString(centroid.y) + ")", centroid, 1, 1, Scalar(255, 0, 0), 2);
			cout << " The centroid of the eel is " << centroid.x << " " << centroid.y << std::endl;
...
int main(){
	ofstream fout("C://Users//gautam//Downloads//eelLog.txt", ios::trunc);
	fout << "\nLogging times of eel activity...\n\n\n";
	VideoCapture cap;
	cap.open("C://Users//gautam//Downloads//GP010191.MP4");
	...
	cv::Mat leftTank;
	cv::Mat rightTank;
	rightTank = cv::Mat(imgTmp, boundrec1).clone();
	leftTank = cv::Mat(imgTmp, boundrec2).clone();
	//imshow("left Tank", leftTank);
	//imshow("right Tank", rightTank);

	Mat baseLeftFrame, baseRightFrame;
	baseLeftFrame = imread("C://Users//gautam//Downloads//leftTank.jpg");
	baseRightFrame = imread("C://Users//gautam//Downloads//rightTank.jpg");
	waitKey(0);
	//int frameCount = 0;
	...
	while (1){
		...
		while (cap.get(CV_CAP_PROP_POS_FRAMES)<cap.get(CV_CAP_PROP_FRAME_COUNT) - 1){
			...
			cv::cvtColor(baseLeftFrame, grayImage2, COLOR_BGR2GRAY);
			
			//convert frame2 to gray scale for frame differencing
			cv::absdiff(grayImage2, grayImage1, differenceImage);
			//perform frame differencing with the sequential images. This will output an "intensity image"
			//do not confuse this with a threshold image, we will need to perform thresholding afterwards.
		
			cv::threshold(differenceImage, thresholdImage, SENSITIVITY_VALUE, 255, THRESH_BINARY);
			int erode_erosion_size = 2;
			Mat erode_element = getStructuringElement(MORPH_RECT,
				Size(2 * erode_erosion_size + 1, 2 * erode_erosion_size + 1),
				Point(erode_erosion_size, erode_erosion_size));

			erode(thresholdImage, thresholdImage, erode_element);
			int dilate_erosion_size = 3;
			Mat dilate_element = getStructuringElement(MORPH_RECT,
				Size(2 * dilate_erosion_size + 1, 2 * dilate_erosion_size + 1),
				Point(dilate_erosion_size, dilate_erosion_size));
			dilate(imgThresholded, imgThresholded, dilate_element);
			...
			if (trackingEnabled){
				...
				if (searchForMovement(thresholdImage, frame1)){
					if (!hasEel) {
						...
						int minutes = x % 60;
						x /= 60;
						cout << minutes << ":" << seconds << "\t|\tAt least one eel detected" << endl;
						fout << minutes << ":" << seconds << "\t|\tAt least one eel detected" << endl;
					}
					hasEel = true;
				}
				else {
					if (hasEel) {
						...
						int minutes = x % 60;
						x /= 60;
						cout << minutes << ":" << seconds << "\t|\tAll eels gone!" << endl;
						fout << minutes << ":" << seconds << "\t|\tAll eels gone!" << endl;
					}
					hasEel = false;
				}
			}
...
void findContour(Mat& imgThreshold)
{
	...
	//cout << "The contours detected is: " << contours.size() << endl;
